...
{
    public class AHSettingsWindow : EditorWindow
    {
        private static AHSettingsWindow m_window;
        ...
        public static AHSettingsWindow Instance
        {
            get
            {
                if (m_window != null)
        ...
        private static AHSettingsWindow initializeWindow()
        {
            m_IntialGUIColor = GUI.color;

            m_window = EditorWindow.GetWindow<AHSettingsWindow>();
            m_window.Show();
        ...
        private void OnGUI()
        {
            ...
            if (settings == null)
            {
                settings = AHSettings.LoadSettings();
            }
            ...
            AHSettingsWindow.Instance.Repaint();

            EditorGUILayout.Separator();
            GUILayout.Label("This is the settingswindow for Asset Helper! " + System.Environment.NewLine + "-Choose folders, types or filenames to exclude when scanning the project", EditorStyles.boldLabel);
            ...
            EditorGUILayout.BeginHorizontal();
            AHSerializableSystemType selectedType = null;

            if (Selection.activeObject)
                selectedType = new AHSerializableSystemType(Selection.activeObject.GetType());
            ...
            if (GUILayout.Button(validSelection ? "Exclude selected type" : "No valid type selected", GUILayout.Width(btnMinWidthLarge)))
            {
                if (validSelection)
                {
                    AHSettings.ExcludeType(selectedType);
                }
            }
            ...
            if (bHasHitEnter || GUILayout.Button(validSelection ? "Exclude substring" : "No valid search string", GUILayout.Width(btnMinWidthLarge)))
            {
                if (validSelection)
                {
                    AHSettings.ExcludeSubstring(m_excludeSubstringInput);
            ...
            EditorGUILayout.EndHorizontal();
            if (validSelection)
                GUILayout.Label(string.Format("Will exclude any asset with \"{0}\" in its path/name (might make asset Helper perform slower)", m_excludeSubstringInput));
            ...
            if (GUILayout.Button(validSelection ? "Exclude selected asset" : "No project asset selected", GUILayout.Width(btnMinWidthLarge)))
            {
                if (validSelection)
                {
                    AHSettings.ExcludeIndividualAssetWithObject(Selection.activeObject);
                }
            }
